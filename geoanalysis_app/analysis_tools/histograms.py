...
def to_datatime(startingDate):
    data_time = datetime.strptime(startingDate, "%m/%d/%Y %I:%M:%S %p")
    return data_time

def count_distance_per_hour(distances, start_time, end_time, distance):
        ...
        if duration_from_start_to_end == 0:
            ...
            if duration_time_hours<duration_from_start_to_end:
                ...
                while start_hour_id < end_hour_id:
                    distances[start_hour_id]+=(duration_time/duration_from_start_to_end)*distance_per_second
                    start_hour_id +=1
...
def histogram_distance_per_hour(data_df):
    ...
    for row in data_df.itertuples():
        ...
        end_time = row[3]
        distance = row[4]
        distances = count_distance_per_hour(distances, start_time, end_time, distance)

    distance_per_hours= list(zip( [str(i)+":00"+"-"+str(i+1)+":00" for i in range(24)], distances) )

    fig = plt.figure()
    df = pd.DataFrame(distance_per_hours, columns=['time', 'distance'])
    df.plot(kind='bar', x='time')
